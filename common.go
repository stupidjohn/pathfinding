...
func vSqr(v Vert) float32 {
	return v.X*v.X + v.Y*v.Y + v.Z*v.Z
}

// take y from (ab x ac)
func triArea2D(a, b, c Vert) float32 {
	var (
		abx = b.X - a.X
		abz = b.Z - a.Z
		acx = c.X - a.X
...
func distPtSegSqr2D(pt, p, q Vert) (t float32, ds float32) {
	var (
		pqx = q.X - p.X
		pqz = q.Z - p.Z
		dx  = pt.X - p.X
...
func vInter(a, b Vert, k float32) Vert {
	return Vert{
		X: a.X + (b.X-a.X)*k,
		Y: a.Y + (b.Y-a.Y)*k,
		Z: a.Z + (b.Z-a.Z)*k,
...
func vHeightOnTriangle(p, a, b, c Vert) (h float32, ok bool) {
	var (
		ab = vSub(b, a)
		ac = vSub(c, a)
		ap = vSub(p, a)
